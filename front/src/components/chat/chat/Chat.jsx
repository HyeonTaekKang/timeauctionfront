...
import '../../../css/components/chat/chat/Chat.css'
import {api, apii, chatApi, noApi, noTokenAuctionApi, noTokenProductApi} from "../../../api/axios";
...
const Chat = ({
                  ...
                  auctionId,
                  productId,
                  updateChatRoomLatestMessageAndSendDateTime,
                  moveChatRoomToTop,
                  previousRoomId,
                  onClose
              }) => { // updateChatRooomLatestMessageAndSendTime : "ProductChatRoom 컴포넌트에서 채팅방의 최신 메세지와 보낸 시간을 update합니다"
    ...
    useEffect(() => {
        const handleSubscription = async () => {
            if (chatRoomId && stompClient) {
                ...
                await fetchChatHistory(chatRoomId); // 채팅 기록 가져오기

                if (roomType === 'product' && productId) {
                    await fetchProductDetails(productId);
                }
                if (roomType === 'auction' && auctionId) {
    ...
    }, [chatRoomId, stompClient, roomType, productId]); // chatRoomId 의존성 제거

    const fetchAuctionDetails = async (id) => {
        try {
            const response = await noApi.get(`/auction/${id}`);
            const auctionDetail = response.data;
            setRoomDetail(auctionDetail);
        } catch (error) {
    ...
    const fetchProductDetails = async (id) => {
        try {
            const response = await noApi.get(`/product/${id}`);
    ...
    const connect = () => {
        if (!stompClient) {
            ...
            newStompClient.connect(
                {chatRoomId: chatRoomId, roomType: roomType, Authorization: `Bearer ${accessToken}`}, // 헤더 추가
                function (frame) {
    ...
    const fetchChatHistory = async () => {
        setFetchChatloading(true); // 로딩 시작
        try {
            const response = await chatApi.get(`/chatMessage/recent/${chatRoomId}/${roomType}?page=1&size=20`);
            setMessages(response.data.data);
            console.log(response.data.data);
        } catch (error) {
            console.log(error);
        } finally {
            setFetchChatloading(false); // 로딩 끝
        }
    }
    ...
    const subscribe = async (newChatRoomId) => {
        ...
        const newSubscription = await stompClient.subscribe(`/topic/chat/${newChatRoomId}`, (messageOutput) => {
                ...
                if (receivedMessageObject.contentType === 'ENTER') {
                    ...
                    updateChatRoomLatestMessageAndSendDateTime(newChatRoomId, receivedMessageObject.message, receivedMessageObject.sendDateTime);
    ...
    const sendNotification = async (receivedMessageObject) => {
        try {
            // 알림을 전송하는 API를 호출합니다.
            await chatApi.post(`/chatMessage/notification/${roomType}`, receivedMessageObject);
        } catch (error) {
    ...
    return (
        <div className="chat-room">
            ...
            <div className="chat-header">
                {/* 경매 정보 표시 */}
                {roomType === 'auction' && roomDetail && (
                    <div className="chat-auction-detail">
                        <div className="chat-auction-detail-container">
                            <img
                                src={`https://dsi4a4vms3mme.cloudfront.net/` + `${roomDetail.thumbNailUrl.replace('origin/', 'thumbnail/')}`}
                                alt="Thumbnail"
                                className="chat-auction-thumbnail"
                            />
                            <div className="chat-auction-info">
                                <div className="chat-auction-title"
                                     dangerouslySetInnerHTML={{__html: roomDetail.title}}/>
                                <div className="chat-auction-nickname">경매인: {roomDetail.nickName}</div>
                                {/* 경매인 표시 */}
                                <p className="chat-auction-times">
                                    {formatAuctionTime(roomDetail.startTime)} - {formatAuctionTime(roomDetail.endTime)} {/* 시간 형식 변경 */}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="chat-product-detail-container">
                            ...
                            <div className="chat-product-info">
                                <div className="chat-product-title"
                                     dangerouslySetInnerHTML={{__html: roomDetail.productTitle}}></div>
                                <p className="chat-product-seller-nickname">판매인: {roomDetail.sellerNickName}</p>
                            </div>
                        </div>
                        <span className="close-chat" onClick={onClose}>
                            <IoClose size={20}/>
